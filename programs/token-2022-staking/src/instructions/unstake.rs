...
impl<'info> Unstake<'info> {
    ...
    fn reward_to_staker(&mut self, bumps: &UnstakeBumps) -> Result<()> {
        ...
        // in solana each slot is intended to produce one block, so I pass in slot as the number of blocks
        let reward_amount = self.stake_info.reward
            .checked_add(
                self.pool.calculate_reward_base_on_blocks(
                    self.stake_info.last_deposit_slot,
                    Clock::get()?.slot,
    ...
    fn update_stake_info(&mut self, amount: u64, bumps: &UnstakeBumps) -> Result<()> {
        ...
        if new_amount.eq(&0) {
            ...
            // Close the stake_info account and return rent-exempt lamports to the staker
            let lamports = **self.stake_info.to_account_info().lamports.borrow();
            **self.stake_info.to_account_info().lamports.borrow_mut() = 0;
            **self.signer.to_account_info().lamports.borrow_mut() += lamports;
