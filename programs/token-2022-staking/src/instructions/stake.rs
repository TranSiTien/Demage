...
impl<'info> Stake<'info> {
    ...
    fn update_stake_info(&mut self, amount: u64) -> Result<()> {
        ...
        if self.stake_info.amount.gt(&0) {
            self.stake_info.reward = self.pool.calculate_reward_base_on_blocks(
                self.stake_info.last_deposit_slot,
                Clock::get()?.slot,
                self.stake_info.amount
            );
            self.stake_info.amount = self.stake_info.amount
                .checked_add(amount)
        ...
        self.stake_info.stake_mint = self.stake_mint.to_account_info().key();
        Ok(())
    }
    ...
    fn deposit(&mut self, amount: u64) -> Result<()> {
        ...
        transfer_checked(
            CpiContext::new(self.stake_token_program.to_account_info(), TransferChecked {
